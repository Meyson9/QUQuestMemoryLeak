...
let app = (key, selStart, selLocal, selLocalRepl, openAll, viosPath = 'assets/voise/samCh.mp3') => {
  ...
  recognizer.onerror = ({
    ...
    switch (error) {
      ...
      case 'network':
        Object(_services_LitlModules__WEBPACK_IMPORTED_MODULE_4__["openTextUserError"])('network', 'Отсутствует соединение к интернету!');
        break;
      // case 'aborted':
    ...
    if (selectorQuestionopen !== selectorQuestionLevel || !selectorQuestionopen) {
      ...
      if (global.mobaleMOde && width <= 775) {
        ...
        if (global.mobaleMOde && width <= 775) {
          ...
          const iconMaterialOpenAll = document.querySelector('.icon-material_gp');
          const elementList = document.querySelector('.gp_segment');
          elementList.removeEventListener('click', _services_LitlModules__WEBPACK_IMPORTED_MODULE_2__["scrollDown"]);
          elementList.removeEventListener('click', () => {
          ...
          iconMaterialOpenAll.parentElement.setAttribute('data-tooltip', "Показать все");
          iconMaterialOpenAll.classList.remove('icon-material_scrollDown'); // document.querySelector('.gp_segment').addEventListener('click',()=>{clickIphone(selectorListQ)},{once:true});
        } else {
          console.log('пиздося');
        }
      ...
      if (global.mobaleMOde && width <= 775) {
        name(numberQuestion, index); // console.log('');
      } else {
        // console.log('PC');
        setTimeout(() => {
          pcsicle();
    ...
    function name(numberQuestion, index) {
      ...
      function chicle() {
        ...
        if (p >= selectorListQ) {
          ...
          index = 0; // console.log('цыкл не разрыв');
